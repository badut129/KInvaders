...
class CombatantMoveComp(private val size: Point, private var act1Interval: Long, private var act2Interval: Long, private var speed: Int, var allyList: ArrayList<GameObject>) : IMoveComponent {
    ...
    override fun update(fps: Long, gameObject: GameObject) {
        ...
        if (gameObject.moving == GameObject.left && gameObject.position.left > 0) {
            gameObject.position.left -= speed / fps
        }

        else if (gameObject.moving == GameObject.right && gameObject.position.right < size.x) {
            gameObject.position.left += speed / fps
        }
        ...
        if(gameObject.action1) {
            ...
            bullet.moving = GameObject.forward
        ...
        if(gameObject.action2) {
            ...
            bullet.moving = GameObject.backward
            var gObjIt = allyList.listIterator()
            gObjIt.add(bullet)
